...
/**
 * Scraper implementation for Amazon. This class handles constructing search URLs, executing the
 * search, parsing the results, and returning a list of products.
 */
public class AmazonScraper implements Scraper {
  ...
  public void parseResults(Document doc, String brand, String item) {
    ...
    System.out.println(
        "Found " + items.size() + " items from Amazon"); // Debug number of items found
    ...
    for (int i = 0; i < items.size(); i++) {
      ...
      String whole = itemElement.select(".a-price-whole").text()
          .replaceAll("[^0-9]", "");  // Removing non-numeric characters

      // Extract the fractional part of the price from the current item element
      String fraction = itemElement.select(".a-price-fraction").text()
          .replaceAll("[^0-9]", "");  // Removing non-numeric characters
      ...
      if (!title.isEmpty() && !whole.isEmpty()) {
        if (title.toLowerCase().contains(item.toLowerCase()) && title.toLowerCase()
